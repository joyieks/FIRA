...
export default function AChatPage({ contact, onBack }) {
      ...
      >
        {messages.map((msg) => (
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            className={`mb-4 ${msg.sender === 'admin' ? 'items-end' : 'items-start'}`}
          >
            <View className={`max-w-[80%] ${msg.sender === 'admin' ? 'bg-gray-800' : 'bg-gray-100'} rounded-2xl px-4 py-3`}>
              {msg.sender !== 'admin' && (
                <Text className="text-xs font-medium text-gray-600 mb-1">
                  {getSenderName(msg.sender)}
                </Text>
              )}
              <Text className={`text-base ${msg.sender === 'admin' ? 'text-white' : 'text-gray-800'} ${msg.isDeleted ? 'italic text-gray-500' : ''}`}>
                {msg.text}
                {msg.isEdited && !msg.isDeleted && (
                  <Text className="text-xs text-gray-400 ml-2">(edited)</Text>
                )}
              </Text>
                             <View className={`flex-row items-center mt-2 ${msg.sender === 'admin' ? 'justify-end' : 'justify-start'}`}>
                 <Text className={`text-xs ${msg.sender === 'admin' ? 'text-gray-300' : 'text-gray-500'}`}>
