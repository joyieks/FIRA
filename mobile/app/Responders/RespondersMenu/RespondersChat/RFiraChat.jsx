...
export default function RFiraChat() {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      if (!currentResponderId || !currentStationId) {
        console.log('❌ Missing required data - currentResponderId:', currentResponderId, 'currentStationId:', currentStationId);
        return;
      }
      ...
      try {
        ...
        if (adminUsers && !adminError) {
          ...
          console.log('⚠️ Admin users error or empty, adding fallback admin');
          // Add fallback admin
          allContactsList.push({
            id: 'fallback-admin-1',
            name: 'Command Center',
        ...
        if (stationUsers && !stationError) {
          ...
          console.log('⚠️ Station users error or empty, adding fallback station');
          // Add fallback station
          allContactsList.push({
            id: 'fallback-station-1',
            name: 'Fire Station',
            email: 'station@fira.com',
            type: 'station',
            avatar: 'F',
            lastMessage: '',
            lastMessageTime: null,
      ...
      unreadCount: 0,
        ...
        console.log('✅ Total contacts count:', allContactsList.length);
        console.log('✅ Admin contacts:', allContactsList.filter(c => c.type === 'admin').length);
        console.log('✅ Station contacts:', allContactsList.filter(c => c.type === 'station').length);
        console.log('✅ Responder contacts:', allContactsList.filter(c => c.type === 'responder').length);
