...
export default function RChatPage({ contact, onBack }) {
  ...
  const [messages, setMessages] = useState([
    { id: 1, text: 'Responder Team Alpha reporting', sender: 'responder', timestamp: '10:30 AM', isRead: true },
    { id: 2, text: 'We have 2 units available', sender: 'responder', timestamp: '10:32 AM', isRead: true },
    { id: 3, text: 'Perfect, keep them ready', sender: 'responder', timestamp: '10:33 AM', isRead: true },
    { id: 4, text: 'Any emergency calls?', sender: 'responder', timestamp: '10:35 AM', isRead: false }
  ]);
      ...
      >
        {messages.map((msg) => (
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            className={`mb-4 ${msg.sender === 'responder' ? 'items-end' : 'items-start'}`}
          >
            <View className={`max-w-[80%] ${msg.sender === 'responder' ? 'bg-gray-800' : 'bg-gray-100'} rounded-2xl px-4 py-3`}>
              {msg.sender !== 'responder' && (
                <Text className="text-xs font-medium text-gray-600 mb-1">
                  {getSenderName(msg.sender)}
                </Text>
              )}
              <Text className={`text-base ${msg.sender === 'responder' ? 'text-white' : 'text-gray-800'} ${msg.isDeleted ? 'italic text-gray-500' : ''}`}>
                {msg.text}
                {msg.isEdited && !msg.isDeleted && (
                  <Text className="text-xs text-gray-400 ml-2">(edited)</Text>
                )}
              </Text>
                             <View className={`flex-row items-center mt-2 ${msg.sender === 'responder' ? 'justify-end' : 'justify-start'}`}>
                 <Text className={`text-xs ${msg.sender === 'responder' ? 'text-gray-300' : 'text-gray-500'}`}>
