...
export default function RChatPage({ contact, onBack }) {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        setIsLoading(true);
        console.log('🔍 Fetching messages between responder:', userData.uid, 'and contact:', contact.id);
        
        const { data: messagesData, error } = await supabase
          .from('messages')
          .select('*')
          .or(`and(sender_id.eq.${userData.uid},receiver_id.eq.${contact.id}),and(sender_id.eq.${contact.id},receiver_id.eq.${userData.uid})`)
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      .channel(`messages:${userData.uid}:${contact.id}`)
      .on('postgres_changes', {
        event: 'INSERT',
        schema: 'public',
        table: 'messages',
        filter: `or(and(sender_id.eq.${userData.uid},receiver_id.eq.${contact.id}),and(sender_id.eq.${contact.id},receiver_id.eq.${userData.uid}))`
      }, (payload) => {
      ...
      >
        {isLoading ? (
          ...
          messages.map((msg, index) => {
            const isMine = msg.sender_id === userData?.uid;
